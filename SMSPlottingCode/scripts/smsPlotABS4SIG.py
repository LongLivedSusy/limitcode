...
class smsPlotABS(object):
    ...
    def __init__(self, modelname, histo, sig1Limits, sig2Limits, sig3Limits, sig4Limits, energy, lumi, preliminary, boxes, label):
        self.standardDef(modelname, histo, sig1Limits, sig2Limits, sig3Limits, sig4Limits, energy, lumi, preliminary, boxes)
        self.LABEL = label
        self.c = rt.TCanvas("cABS_%s" %label,"cABS_%s" %label,300,300)
        self.histo = histo

    def standardDef(self, modelname, histo, sig1Limits, sig2Limits, sig3Limits, sig4Limits, energy, lumi, preliminary, boxes):
        # which SMS?
        self.model = sms(modelname)
        self.SIG1 = sig1Limits
        self.SIG2 = sig2Limits
        self.SIG3 = sig3Limits
        self.SIG4 = sig4Limits
        self.lumi = lumi
        self.energy = energy
    ...
    def DrawLegend(self):
        ...
        LSig4 = rt.TGraph(2)
        LSig4.SetName("LSig4")
        LSig4.SetTitle("LSig4")
        LSig4.SetLineColor(color(self.SIG4['colorLine']))
        LSig4.SetLineStyle(7)
        LSig4.SetLineWidth(3)
        LSig4.SetPoint(0,self.model.Xmin+63*xRange/100, self.model.Ymax-1.35*yRange/100*10)
        LSig4.SetPoint(1,self.model.Xmin+70*xRange/100, self.model.Ymax-1.35*yRange/100*10)
        
        textSig4 = rt.TLatex(self.model.Xmin+71*xRange/100, self.model.Ymax-1.50*yRange/100*10, "@ 300 fb^{-1}")
        textSig4.SetTextFont(42)
        textSig4.SetTextSize(0.035)
        textSig4.Draw()
        self.c.textSig4 = textSig4
    ...
    def DrawLines(self):
        ...
        # sig4
        self.SIG4['nominal'].SetLineColor(color(self.SIG4['colorLine']))
        self.SIG4['nominal'].SetLineStyle(7)
        self.SIG4['nominal'].SetLineWidth(3)         
        # DRAW LINES
        self.SIG4['nominal'].Draw("LSAME")
